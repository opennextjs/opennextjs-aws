Since V3 you don't pass build options using args anymore, you need to create a file called `open-next.config.ts` at the root of your project, and export a default object that satisfies the `OpenNextConfig` interface. You can also pass another file using the `--config-path` args.
