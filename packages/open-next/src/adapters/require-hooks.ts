...
export function addHookAliases(aliases: [string, string][] = [], isApp = true) {
  for (const [key, value] of aliases) {
    isApp ? hookPropertyMapApp.set(key, value) : hookPropertyMapPage.set(key, value);
  }
}
...
export function overrideReact(config: NextConfig) {
  addHookAliases(
    [
      ...
      ["react-dom/server.browser", `/var/task/node_modules/react-dom/server.browser`],
    ],
    false
  );
