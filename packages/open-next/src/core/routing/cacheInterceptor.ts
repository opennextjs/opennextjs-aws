...
function getBodyForAppRouter(
  ...
  try {
    ...
    // `rsc` is absent when Next.js did not write the `.rsc` file at build time, i.e. for
    // fallback shells and for PPR routes with a postponed state. Those responses have to
    // be generated by the server, serving an empty payload here would break the router
    // and the empty response could be cached by the CDN.
    ...
    return cachedValue.rsc === undefined
      ? undefined
      : { body: cachedValue.rsc, additionalHeaders: {} };
  }
}
...
async function generateResult(
  ...
  if (cachedValue.type === "app") {
    ...
    if (isDataRequest) {
      ...
      if (appRouterResult === undefined) {
        debug("No RSC payload in the cache entry, falling back to the server");
        return undefined;
      }
